...
//cache handling
SObjectData.prototype.getCacheQuery = function() {
	if (this.cacheConfig.cacheQuery != null) {
		return this.cacheConfig.cacheQuery;
	} else {
		return {queryType:"smart", smartSql:"SELECT {"+this.cacheConfig.cache.soupName+":_soup} FROM {"+this.cacheConfig.cache.soupName+"}", pageSize:this.cacheConfig.pageSize};
	}
}
...
SObjectData.prototype.fetch = function(type,query,callback) {
	...
	if(connection == 'none') {
		console.log('No Connection Found, Trying Cache');
		if( (this.cacheConfig.cacheMode == Force.CACHE_MODE.SERVER_ONLY) ) {
			console.log('SERVER ONLY MODE');
			this.errorHandler({'errorMessage':'No connection'});
		} else {
			type = "cache";
			cacheQuery = this.getCacheQuery();
			console.log(cacheQuery.smartSql);
		}
	}
	...
	Force.fetchSObjects(
		{'type':type,'query':query,'cacheQuery':cacheQuery},this.cacheConfig.cache
...
SObjectData.prototype.sync = function(pointer,callback,_isDelete) {
	...
	cacheMode = this.cacheConfig.cacheMode;
	if(navigator.connection.type == "none" && cacheMode != Force.CACHE_MODE.SERVER_ONLY) {
		cacheMode = Force.CACHE_MODE.CACHE_ONLY;
	} 
	
	Force.syncSObject(method,this.SObjectType,_record.Id,
		attributes,this.fields,this.cacheConfig.cache,cacheMode).then(function(response){
